...
import { WINDOW, BALL, WALL, PADDLE, OPTIONS } from "./constants.js";
...
const menu = (options, defaultOption) => {
	...
	while (!shouldWindowClose && !isAValidOption(keyPressed, options.length)) {
		...
		shouldWindowClose = r.WindowShouldClose();
...
const main = () => {
	...
	menu([
		{ name: "startGame", onSelect: play },
		{ name: "exit", onSelect: r.CloseWindow },
	]);
};
